...
namespace Nameday
{
    public static class NamedayRepository
    {
        ...
        public static async Task<List<NamedayModel>> GetAllNamedaysAsync()
        {
            ...
            allNamedaysCache = new List<NamedayModel>();

            Random rnd = new Random((int)DateTimeOffset.Now.Ticks);
            for(int month = 1; month <= 12; month++)
            {
                int days = rnd.Next() % 10;
                for(int day = 1; day < days; day++)
                {
                    allNamedaysCache.Add(new NamedayModel(month, rnd.Next() % 30, new string[] { "alpha" + month + ":" + day }));
                }
            }
